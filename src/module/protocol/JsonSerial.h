...
#define XX( name ) this->m_current[names[this->count]] = name;
        ...
        if constexpr ( IsContainerType< T1 >::value )
        {
            if ( typeid( arg[0] ) == typeid( int ) )
            {
                arr_type.push_back( type::Int );
            }
            else if ( typeid( arg[0] ) == typeid( unsigned int ) )
            {
...
#undef XX
    ...
    void judgeType( std::vector< std::string > names )
    {
        ...
        else if ( std::is_same_v< T1, unsigned int > )
        {
    ...
    void judgeType1( T1& arg )
    {
        if constexpr ( IsContainerType< T1 >::value )
        {
            ...
            for ( int i = 0; i < sz; i++ )
            {
                ...
                else if constexpr ( std::is_same_v< typename T1::value_type, unsigned int > )
                {
        ...
        else if constexpr ( std::is_pointer< T1 >() )
        {
            ...
            for ( int i = 0; i < sz; i++ )
            {
                ...
                else if constexpr ( std::is_same_v< decltype( arg[0] ), unsigned int > )
                {
